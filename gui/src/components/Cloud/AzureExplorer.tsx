...
export default function AzureExplorer() {
  const { 
    services, 
    isLoading, 
    error,
  ...
  const resourceGroups = ['all', ...new Set(services.map(s => s.region))]
  const types = ['all', 'compute', 'storage', 'database', 'serverless']

  const filteredServices = services.filter(s => 
    (selectedResourceGroup === 'all' || s.region === selectedResourceGroup) &&
  ...
  return (
    <div className="cloud-explorer">
      ...
      <div className="cloud-filters">
        <select value={selectedResourceGroup} onChange={(e) => setSelectedResourceGroup(e.target.value)}>
          {resourceGroups.map(rg => (
            <option key={rg} value={rg}>{rg === 'all' ? 'All Resource Groups' : rg}</option>
          ))}
        </select>
